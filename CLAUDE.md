...
- [ ] **Homepage "Curated for every occasion" needs 4 real collections** (Slow Mornings, Entertaining, Sunday Roast, High Tea) before its links are real — built as a custom block (`blocks/lambruk-occasion-card.liquid`) with each card's link defaulting to `/collections/all` as a placeholder, same shape as the Shop-column links above. Revisit those 4 links once the client has created the collections (2026-08-21).
- [ ] **Homepage "Seasonal Spotlight" product rail has no collection assigned.** Built via `sections/product-list.liquid`'s native `collection` setting, deliberately left blank rather than pointed at a guess. Needs a real "seasonal spotlight" collection created and assigned in the theme editor (2026-08-21).
