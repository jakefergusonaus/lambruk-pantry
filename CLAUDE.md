...
- [ ] **Homepage images — mostly resolved, one still open.** Hero cheeseboard photo (`DSC_2479.jpg`), Cafe intro photo (`Ballina_Cafe_1.jpg`), all 4 Curated Occasion card photos, and "The Lambruk Promise" quote panel illustration (`seedling-gold.png`, resolved earlier, confirmed still present) are all assigned — confirmed directly in the committed `templates/index.json`, not assumed from the editor. **Hero award badge overlay is still unassigned** — no `badge`-related image key found anywhere in the file (2026-09-01).
