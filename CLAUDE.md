...
**The Shopify theme editor and this repo are two writers on the same file, and neither one knows the other exists.** Jake editing content in Admin (images, settings, metafields, anything saved through the theme editor UI) and this repo pushing a locally-edited copy of the same JSON file are both write paths to the same theme, with no merge, no lock, and no warning when they collide — whichever one saves last silently wins, discarding the other's changes with no error. **Before running `shopify theme dev` or any `theme push`, pull first.** After Jake says he's edited something in the theme editor, pull and commit that change before doing anything else — don't queue it behind other work, and don't assume a stale local copy is still current just because nothing in this session touched that file. This has already destroyed four rounds of real, confirmed-saved work this way: the header settings, the Top Sellers collection binding, the Cafe menu URL setting, and the homepage image assignments.

**A written reminder does not stop this — a fifth incident happened while the paragraph above was still being written**, because the dev server from an unrelated task was left running through it. A `shopify theme dev` process left running after its own task is done is invisible in the transcript and keeps silently overwriting anything Jake does in the theme editor for as long as it stays up — "remember to stop it" isn't a control, it's hope. **"No `shopify theme dev` process is running" is now a checked, reported item in every task's completion handoff** (`workflow/PIPELINE.md` §10) — the same status as Theme Check, not a note to self. Five incidents means it needed to be a check, not a sixth rule to remember.

**A sixth incident, same day: the "pull first" rule above is dangerous exactly as written, and it proved it against this session's own work.** `shopify theme pull` (no `--only` flag) syncs in *both* directions, not just remote-to-local — it deletes local files that don't exist on the remote and overwrites local edits that haven't been pushed yet. Mid-build on the hero award badge (2026-09-02), a bare pull run per this rule deleted a new, uncommitted block file outright and reverted two other uncommitted edits back to their last-pushed state. Nothing was permanently lost that time only because the content still existed in the session's own context and could be retyped — a pull an hour later, or in a session without that context, destroys the work with no recovery. The rule as written — "pull first" — says nothing about the state of the local tree it's about to overwrite, which is exactly the gap: a rule that assumes the tree is clean isn't a rule, it's a hope, the same failure shape as the dev-server reminder above. **Amended rule: commit or stash any local changes *before* running `shopify theme pull` — never pull into a dirty working tree. When only a specific file needs to be pulled, scope it with `shopify theme pull --only <path>` rather than syncing the whole theme.** `git status --short` immediately before any pull is now the check, not a "should be clean by now" assumption — see `workflow/PIPELINE.md` §7 and §10.
...
- [ ] **Homepage images — mostly resolved, one still open.** Hero cheeseboard photo (`DSC_2479.jpg`), Cafe intro photo (`Ballina_Cafe_1.jpg`), all 4 Curated Occasion card photos, and "The Lambruk Promise" quote panel illustration (`seedling-gold.png`, resolved earlier, confirmed still present) are all assigned — confirmed directly in the committed `templates/index.json`, not assumed from the editor. **Hero award badge overlay: mechanism built 2026-09-02** (`blocks/_lambruk-award-badge.liquid`, `ARCHITECTURE.md` §58) — the block genuinely didn't exist before this (the earlier "unassigned" note was wrong; there was nothing to assign an image to). Now registered on the hero's content column, positioned correctly at both breakpoints and both `media_position` orientations, renders nothing until an image is set. **Still needs Jake to assign the badge image in the theme editor** — he's already uploaded it to Files. Once he saves, pull `templates/index.json` and commit before anything else, per the standing rule.
