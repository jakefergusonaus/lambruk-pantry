...
**Occasion hero eyebrow colour — gold → white, deliberate departure, 2026-09-09.** `design_handoff_website/`'s occasion collection hero (`Desktop.dc.html`/`Mobile.dc.html`) specifies the eyebrow ("CURATED OCCASION") in gold, `color:#A07037` — confirmed identical to the built value before this change, on both breakpoints. `.lambruk-occasion-hero__eyebrow` in `sections/lambruk-occasion-hero.liquid` now uses `#F7F5F2`, matching the off-white already used by this same component's own `__title` colour, per instruction. **Contrast not yet fully clean:** measured against each of the four occasion collections' own photo + scrim, at the eyebrow's actual rendered position, both breakpoints (8 measurements, WCAG 1.4.3 small-text threshold 4.5:1) — three collections pass at both breakpoints (High Tea, Slow Mornings, Sunday Roast: worst case 4.82:1) and Entertaining passes at 1280px (6.11:1), but **Entertaining at 375px measures as low as 3.71:1 against a light patch of its own photo, failing the requirement.** Left unresolved pending a decision — the photo, the scrim, or the eyebrow's own colour/treatment on this one collection at this one breakpoint needs to change, and picking a different off-white without measuring it first isn't a fix, it's a guess.
